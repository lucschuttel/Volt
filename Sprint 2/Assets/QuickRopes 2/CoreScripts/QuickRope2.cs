...
public class QuickRope2 : MonoBehaviour
{
    ...
    void ExtendRope(float velocity)
    {
        ...
        if (Vector3.Distance(Joints[1].transform.position, Joints[0].transform.position) > (distBetweenJoints*1.5f))
        {
            ...
            AddConfigJoint(go).connectedBody = Joints[1].rigidbody;
    ...
    void UpdatePhysics()
    {
        ...
        for (int r = 0; r < Joints.Count; r++)
        {
            ...
            switch (constraint)
            {
                ...
                case RopeConstraint.X_Y:
                    go.rigidbody.constraints = RigidbodyConstraints.FreezeRotationY | RigidbodyConstraints.FreezeRotationX | RigidbodyConstraints.FreezePositionZ;
                    break;
                case RopeConstraint.Y_Z:
                    go.rigidbody.constraints = RigidbodyConstraints.FreezeRotationY | RigidbodyConstraints.FreezeRotationZ | RigidbodyConstraints.FreezePositionX;
                    break;
                case RopeConstraint.Z_X:
                    go.rigidbody.constraints = RigidbodyConstraints.FreezeRotationZ | RigidbodyConstraints.FreezeRotationX | RigidbodyConstraints.FreezePositionY;
                    break;
            }

            if (solverOverride != -1 && solverOverride > 1)
                go.rigidbody.solverIterationCount = solverOverride;

            go.rigidbody.mass = mass;
            go.rigidbody.angularDrag = angDrag;
            go.rigidbody.drag = drag;
            go.rigidbody.useGravity = useGravity;
        }
        ...
        if (Application.isPlaying)
        {
            ...
            for (int c = 0; c < Joints.Count - 1; c++)
                AddConfigJoint(Joints[c]).connectedBody = Joints[c + 1].rigidbody;
    ...
    ConfigurableJoint AddConfigJoint(GameObject joint)
    {
        ...
        if (Application.isPlaying && joint.collider && colliderType == RopeColliderType.DEFAULT)
